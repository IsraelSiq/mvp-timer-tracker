...
export function goalScore(mvp: EnrichedMVP, mode: GoalMode): number {
  // Considera disponível: janela aberta ou em breve (tem registro e está próximo)
  const isAvailable = mvp.status === 'window-open' || mvp.status === 'soon'
  ...
  switch (mode) {
    case 'mvp-points': {
      ...
      const respawnBonus    = Math.max(0, 200 - mvp.minRespawn)
